...
print("Before TRPROb")
    ...
    ds1 <- dfFilter
        return(ds1[, c('NuTR', 'TRProb', 'TRMean', 'TRSD')])
...
#getVols foRT ral time eal real tiem daat
#getVols function for SD and vols 
getVolsRT <- function(x) {
    m <- as.matrix(x)
    ...
    mlen <- nrow(df)
    df <- mutate(df, PCHG = temp * 100)
    df$AvgRet <- rollapply(df$PCHG, mlen, mean, fill = NA, align = 'right', na.rm = TRUE)

    df$SD20 <- runSD(n = mlen, df$PCHG)
