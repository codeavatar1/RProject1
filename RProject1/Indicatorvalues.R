...
write.csv(symbolslist, file = paste(myenv$tradestationOutDir, "testsymbol1.txt", sep = ''), row.names = FALSE)
...
for (i in 1:rcnt) {
    ...
    #element1dup
    sma20 <- SMA(Cl(ROKU), n = 20)


    rsi14 <- RSI(Cl(ROKU), n = 14, maTyep = 1)
    atr <- ATR(HLC(ROKU))    
